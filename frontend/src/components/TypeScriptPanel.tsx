...
function buildEnumType(name: string, members: string[]): string {
...
export function TypeScriptPanel({ data }: Props) {
  const [tab, setTab] = useState<'enums' | 'interfaces' | 'functions'>('interfaces');

  const enumEntries = Object.entries(data.enums ?? {});
  const allEnumsCode = enumEntries.map(([name, members]) => buildEnumType(name, members)).join('\n\n');
    ...
    <div className={styles.panel}>
      <div className={styles.tabs}>
        {enumEntries.length > 0 && (
          <button
            className={`${styles.tab} ${tab === 'enums' ? styles.active : ''}`}
            onClick={() => setTab('enums')}
          >
            Enums
            <span className={styles.count}>{enumEntries.length}</span>
          </button>
        )}
        <button
      ...
      {tab === 'enums' && (
        <div className={styles.content}>
          <div className={styles.blockHeader}>
            <span className={styles.blockLabel}>All enum types</span>
            <CopyButton text={allEnumsCode} />
          </div>
          {enumEntries.map(([name, members]) => (
            <div key={name} className={styles.block}>
              <div className={styles.blockInnerHeader}>
                <span className={styles.interfaceName}>{name}</span>
                <CopyButton text={buildEnumType(name, members)} />
              </div>
              <pre className={styles.code}>{buildEnumType(name, members)}</pre>
            </div>
          ))}
        </div>
      )}
